...
nvinfer1::ICudaEngine* LazilyDeserializedEngine::get()
{
	...
	memoryConsumption memUsageChecker;
        memUsageChecker.printMemInfo("Start to narrow down the mem usage for engine creation", true);
        std::unique_ptr<IRuntime> runtime{createInferRuntime(sample::gLogger.getTRTLogger())};
	memUsageChecker.printMemInfo("InferRuntime Creation", true);
	...
	memUsageChecker.printMemInfo("Engine deserialization", false);
...
nvinfer1::safe::ICudaEngine* LazilyDeserializedEngine::getSafe()
{
	...
	memoryConsumption memUsageChecker;
	memUsageChecker.printMemInfo("Start to narrow down the mem usage for engine create", true);
