...
bool setUpInference(InferenceEnvironment& iEnv, InferenceOptions const& inference, memoryConsumption& mem_consumption)
{
    int32_t device{};
    ...
    mem_consumption.printMemInfo("Start to narrow down the mem usage after engine creation ", true);
    for (int32_t s = 0; s < inference.streams; ++s)
    {
    ...
    mem_consumption.printMemInfo("ExecutionContext Creation", true);
    if (iEnv.profiler)
    {
    ...
    auto result = FillStdBindings(engine, context, inference.inputs, iEnv.bindings, batch, endBindingIndex)();
    mem_consumption.printMemInfo("Bindings buffer filling", true);
    return result;
}
